...
class ContainerRepository(private val collection: MongoCollection<ContainerModel>) {
    private val replaceOptions = ReplaceOptions().upsert(true)

    fun openChangeStream(pipelines: List<Bson>): ChangeStreamFlow<Document> =
        collection.withDocumentClass<Document>().watch(pipelines)

    suspend fun findByPlayerId(playerId: UUID, projection: Bson): Document? = collection
