...
import plutoproject.feature.teleport.api.paper.events.TeleportEvent
import plutoproject.foundation.common.text.MESSAGE_SOUND
import plutoproject.foundation.common.text.UNUSUAL_ISSUE_OCCURRED
import plutoproject.foundation.common.text.replace
import plutoproject.foundation.common.text.replaceSubTitle
import plutoproject.feature.teleport.paper.moduleScope
import plutoproject.foundation.common.collection.mapKeysAndValues
import plutoproject.foundation.common.coroutine.raceConditional
import plutoproject.foundation.common.time.toFormattedComponent
import plutoproject.feature.teleport.paper.coroutineContext
import plutoproject.foundation.paper.entity.teleportSuspend
import plutoproject.feature.teleport.paper.server
import plutoproject.feature.teleport.api.paper.ChunkLocation
import plutoproject.feature.teleport.paper.getChunkFromSource
import java.util.*
...
class TeleportManagerImpl : TeleportManager {
    private val config by koinInject<TeleportConfig>()
    ...
    private suspend fun Location.searchSafeLoc(options: TeleportOptions): Location? {
        ...
        return raceConditional(
            moduleScope.async { iterateLocations(0..(world.maxHeight - blockY), BlockFace.UP) },
            moduleScope.async { bfs() },
            moduleScope.async { iterateLocations(0..(blockY - world.minHeight), BlockFace.DOWN) }
    ...
    override fun searchSafeLocation(start: Location, options: TeleportOptions?): Location? =
        moduleScope.async { searchSafeLocationSuspend(start, options) }.asCompletableFuture().join()
