...
class ChunkCallbacks(
    ...
    private val index: DisplayInstanceIndex,
    private val imageStore: ImageStore,
    private val imageDataStore: ImageDataStore,
    private val displayInstanceStore: DisplayInstanceStore,
    private val displayRuntime: DisplayRuntimeRegistry,
) {
    suspend fun chunkLoad(world: String, chunk: ChunkKey) {
        ...
        if (invalidImages.isNotEmpty()) {
            logger.log(
                Level.WARNING,
                "Found ${invalidImages.size} image(s) without corresponding data: $invalidImageString",
            )
        }
