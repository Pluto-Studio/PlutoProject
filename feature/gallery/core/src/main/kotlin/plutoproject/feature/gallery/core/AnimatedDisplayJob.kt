...
class AnimatedDisplayJob(
    ...
    private fun nextAwakeAt(wakeStartedAt: java.time.Instant): java.time.Instant {
        if (maxFramePerSecond == -1) {
            return clock.instant()
        }

        val wakeFinishedAt = clock.instant()
        val elapsedMillis = java.time.Duration.between(wakeStartedAt, wakeFinishedAt).toMillis().coerceAtLeast(0)
