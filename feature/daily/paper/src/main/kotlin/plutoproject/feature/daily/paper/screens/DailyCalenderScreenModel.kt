...
import plutoproject.foundation.common.time.atStartOfMonth
...
class DailyCalenderScreenModel(private val player: Player) : ScreenModel {
    ...
    suspend fun init() {
        isLoading = true
        user = plutoproject.kernel.api.koinGet<Daily>().getUserOrCreate(player.uniqueId)
        loadData()
        isLoading = false
    }
    ...
    suspend fun loadData() {
        ...
        accumulatedDays = plutoproject.kernel.api.koinGet<Daily>().getAccumulationBetween(player.uniqueId, start, end)
        loadedHistories.clear()
        loadedHistories.addAll(plutoproject.kernel.api.koinGet<Daily>().getHistoryByTime(player.uniqueId, start, end))
        isLoading = false
    }
