...
import plutoproject.foundation.paper.inventory.isFull
...
object HeadCommand {
    private val config by koinInject<HeadConfig>()
    private val profileLookup = currentModuleContext().services.getService<ProfileLookup>()
    private val plugin = (currentModuleContext() as PaperModuleContext).plugin
    ...
    suspend fun CommandSender.head(@Argument("player", suggestions = "players") player: String?) = ensurePlayer {
        ...
        val eco = plugin.server.servicesManager.getRegistration(Economy::class.java)?.provider
            ?: return@ensurePlayer
            ...
            val source = if (name != targetName) {
                ...
                runCatching {
                    withTimeout(10.seconds) {
                        profileLookup.lookupByName(targetName)?.uuid
                    }
                }.onFailure {
