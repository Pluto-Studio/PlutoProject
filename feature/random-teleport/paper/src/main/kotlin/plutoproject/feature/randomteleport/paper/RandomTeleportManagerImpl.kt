...
import plutoproject.feature.randomteleport.api.paper.*
import plutoproject.feature.randomteleport.api.paper.events.RandomTeleportEvent
import plutoproject.feature.teleport.api.paper.ManagerState
...
import plutoproject.feature.randomteleport.paper.plugin
import plutoproject.feature.randomteleport.paper.server
...
class RandomTeleportManagerImpl : RandomTeleportManager {
    ...
    override val defaultOptions: RandomTeleportOptions = RandomTeleportOptions(
        ...
        chunkPreserveRadius = if (config.default.chunkPreserveRadius >= 0) config.default.chunkPreserveRadius else teleportManager.defaultTeleportOptions.chunkPrepareRadius,
    ...
    override suspend fun launchSuspend(
        ...
        withContext(Dispatchers.Default) {
            ...
            if (location == null) {
                if (prompt) {
                    player.showTitle(RANDOM_TELEPORT_FAILED_TITLE)
                    player.playSound(RANDOM_TELEPORT_FAILED_SOUND)
