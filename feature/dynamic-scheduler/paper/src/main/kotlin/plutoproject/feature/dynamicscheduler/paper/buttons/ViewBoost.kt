package plutoproject.feature.dynamicscheduler.paper.buttons
...
fun ViewBoost() {
    ...
    var state by mutableStateOf(plutoproject.kernel.api.koinGet<DynamicScheduler>().getViewDistanceLocally(player))
    Item(
        ...
        modifier = Modifier.clickable {
            ...
            when (state) {
                DynamicViewDistanceState.ENABLED -> {
                    plutoproject.kernel.api.koinGet<DynamicScheduler>().setViewDistance(player, false)
                    player.playSound(UI_TOGGLE_OFF_SOUND)
                    state = DynamicViewDistanceState.DISABLED
                }

                DynamicViewDistanceState.DISABLED -> {
                    plutoproject.kernel.api.koinGet<DynamicScheduler>().setViewDistance(player, true)
